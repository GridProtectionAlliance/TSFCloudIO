...
namespace ConfigurationSetupUtility
{
    ...
    public partial class App : Application
    {
        ...
        public const string ApplicationExe = "TSFCloudIO.exe";
        public const string ApplicationConfig = "TSFCloudIO.exe.config";
        public const string Manager = "TSFCloudIOManager";
        public const string ManagerExe = "TSFCloudIOManager.exe";
        public const string ManagerConfig = "TSFCloudIOManager.exe.config";
        public const string BaseSqliteConfig = "TSFCloudIO.db";
        public readonly static string SqliteConfigv2 = "TSFCloudIO" + DatabaseVersionSuffix + ".db";
        public const string SqliteSampleData = "TSFCloudIO-SampleDataSet.db";
        public const string SqliteInitialData = "TSFCloudIO-InitialDataSet.db";
        ...
        public App()
        {
            ...
            // Attempt to create an event log source for the TSFCloudIO Manager for authentication logging. This needs to be done
            // here since the CSU runs with administrative privileges and the TSFCloudIO Manager normally does not; also there is
            // a short system delay that exists before you can write to a new event log source after it is first created.
            try
            {
                string applicationName = "TSFCloudIO";

                // Create the event log source based on defined application name for TSFCloudIO if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");

                applicationName = "TSFCloudIO Manager";

                // Create the event log source based on defined application name for TSFCloudIO Manager if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");
            }
            catch (Exception ex)
            {
                m_errorLogger.Log(new InvalidOperationException(string.Format("Warning: failed to create or validate the event log source for the TSFCloudIO Manager: {0}", ex.Message), ex), false);
            }
        }
