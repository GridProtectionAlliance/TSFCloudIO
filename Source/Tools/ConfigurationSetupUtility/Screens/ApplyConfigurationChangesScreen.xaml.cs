...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ApplyConfigurationChangesScreen : IScreen
    {
        ...
        private void InitializeState()
        {
            ...
            m_TSFCloudIOManagerLocalCheckBox.IsEnabled = managerOptionsEnabled;
            m_TSFCloudIOManagerWebCheckBox.IsEnabled = webManagerOptionEnabled;

            // If the options are disabled, they must also be unchecked.
            if (!managerOptionsEnabled)
                m_TSFCloudIOManagerLocalCheckBox.IsChecked = false;

            if (!webManagerOptionEnabled)
                m_TSFCloudIOManagerWebCheckBox.IsChecked = false;

            m_setupHistorianCheckBox.IsChecked = false;

            // Set up the state object with the proper initial values.
            m_state["applyChangesToService"] = m_TSFCloudIOServiceCheckBox.IsChecked.GetValueOrDefault();
            m_state["applyChangesToLocalManager"] = m_TSFCloudIOManagerLocalCheckBox.IsChecked.GetValueOrDefault();
            m_state["applyChangesToWebManager"] = m_TSFCloudIOManagerWebCheckBox.IsChecked.GetValueOrDefault();
        ...
        // Occurs when the user chooses to apply changes to the TSFCloudIO service.
        private void TSFCloudIOServiceCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = true;
        }

        // Occurs when the user chooses to not apply changes to the TSFCloudIO service.
        private void TSFCloudIOServiceCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = false;
        }

        // Occurs when the user chooses to changes to the local TSFCloudIO Manager application.
        private void TSFCloudIOManagerLocalCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the local TSFCloudIO Manager application.
        private void TSFCloudIOManagerLocalCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = false;
        }

        // Occurs when the user chooses to apply changes to the TSFCloudIO Manager web application.
        private void TSFCloudIOManagerWebCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToWebManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the TSFCloudIO Manager web application.
        private void TSFCloudIOManagerWebCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
