...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class NodeSelectionScreen : UserControl, IScreen
    {
        ...
        private void UpdateDataGrid()
        {
            ...
            {
                m_infoTextBlock.Text = "Please select the node you would like the TSFCloudIO to use.";
            }
            else
            {
            ...
            if (applyChangesToService)
                nodeId = GetNodeIdFromConfigFile("TSFCloudIO.exe.config");
            else if (applyChangesToLocalManager)
                nodeId = GetNodeIdFromConfigFile("TSFCloudIOManager.exe.config");
            else
                nodeId = Guid.Empty;
            ...
            if (applyChangesToService)
                TryAddNodeInfo("TSFCloudIO.exe.config");

            if (applyChangesToLocalManager)
                TryAddNodeInfo("TSFCloudIOManager.exe.config");
        ...
        private IDbConnection GetConnectionFromConfigFile()
        {
            ...
            string configFileName = Directory.GetCurrentDirectory() + "\\TSFCloudIO.exe.config";
        ...
        {
            return GetValueOfSystemSetting("TSFCloudIO.exe.config", "CompanyName");
        }
        ...
        // Occurs when the user selects a different node to be used by the TSFCloudIO.
