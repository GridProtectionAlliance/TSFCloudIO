...
SET defaulttarget=%LOCALAPPDATA%\Temp\TSFCloudIO
IF "%remote%" == "" SET remote=git@github.com:GridProtectionAlliance/tsfcloudio.git
...
SET targettools=%target%\Source\Applications\TSFCloudIO\TSFCloudIOSetup
...
FOR /R "%sourceschema%" %%x IN (GSFSchema.*) DO REN "%%x" "TSFCloudIO.*"
FOR /R "%sourceschema%" %%x IN (GSFSchema-InitialDataSet.*) DO REN "%%x" "TSFCloudIO-InitialDataSet.*"
FOR /R "%sourceschema%" %%x IN (GSFSchema-SampleDataSet.*) DO REN "%%x" "TSFCloudIO-SampleDataSet.*"
...
"%replace%" /r /v "%targetschema%\*.sql" GSFSchema TSFCloudIO
...
"%replace%" /r /v "%targetschema%\*db-update.bat" GSFSchema TSFCloudIO
